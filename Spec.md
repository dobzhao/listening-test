...
| 阶段 | 默认时长 | UI 显示 |
|------|---------|---------|
| INTRO | 10s | **仅第 5 题前显示一次**（本部分开场介绍，纯文字 + 倒计时） |
...
除 **第 19 题录音 90 秒**外，上述 14 个时长（INTRO ×5 / PREPARE / 静音间隔 / ANSWERING / FILL_BLANK / RECALL_PREP）均可在 **设置 → 流程时长** Tab 中调整，单位为秒，内部以毫秒存储：
...
|------|------|----------|
| `intro_ms` | 10s | 1-4 题前 |
| `group_intro_ms` | 10s | 5-14 题前 |
| `retell_intro_ms` | 10s | 15-18 题前 |
| `retell_play3_intro_ms` | 10s | 15-18 题 PLAYING #3 前（FILL_BLANK 之后） |
| `retell_q19_intro_ms` | 10s | 19 题录音前（默读准备之后） |

### 3.5.1 开场介绍文案可配置说明

5 段介绍都是**纯文字 + 倒计时**，不合成也不播放语音。文案在 **设置 → 开场介绍** Tab 中编辑：

- 内置默认文案，每段提供单独的「恢复默认」，并支持「全部恢复默认」
- 修改后点击右上角「保存配置」写入 `config.json::intro`
- **把某段时长设为 0 秒（或清空该段文案）即可跳过对应介绍**
- 介绍期间「下一题」按钮隐藏；1-14 题的题干与选项在介绍期间不显示，15-19 题的挖空表格在介绍期间也不显示
...
6. **流程时长**：答题过程每个阶段等待时长（14 项）+ 恢复默认
7. **开场介绍**：5 段介绍文案可编辑 + 单段/全部恢复默认（详见 §3.5.1）
8. **难度**：三档文字模板编辑 + 档位选择 + 「自动切换难度」开关 + 「重置自适应状态」按钮（v1.1+，详见 §7.5）
9. **设备测试**：可选输入/输出设备 + 录音测试 + 测试音播放
...
5. **19 题录音时长固定 90 秒**：受 STT/LLM 判分稳定性约束，不可在 UI 中调整；其余 13 个流程时长均可在「设置 → 流程时长」中调整
